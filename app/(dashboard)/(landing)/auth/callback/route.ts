...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    try {
      ...
      if (session && session.user && session.user.app_metadata?.provider === 'google') {
        ...
        await supabase.auth.updateUser({
          data: {
            ...
            // Explicitly check for landlord role from params
            user_role: searchParams.get('user_role') === 'landlord' ? 'landlord' : 'tenant'
          }
        })
      }
      ...
      const dashboardPath = userRole === 'landlord' 
        ? '/landlord-dashboard' 
        : userRole === 'admin'
          ? '/admin-dashboard'
          : '/tenant-dashboard';
      ...
      if (isLocalEnv) {
        return NextResponse.redirect(`${origin}${next}?user_role=${userRole || 'tenant'}`)
      } else if (forwardedHost) {
        return NextResponse.redirect(`https://${forwardedHost}${next}?user_role=${userRole || 'tenant'}`)
      } else {
        return NextResponse.redirect(`${origin}${next}?user_role=${userRole || 'tenant'}`)
      }
    } catch (error) {
