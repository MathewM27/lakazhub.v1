...
export default function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const isDismissedRecently = Date.now() - dismissedTime < 7 * 24 * 60 * 60 * 1000; // 7 days
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 md:left-auto md:bottom-8 md:right-8 z-50 bg-black border border-white/20 rounded-xl shadow-xl p-4 backdrop-blur-lg max-w-md">
      <div className="flex items-center">
        ...
        <div className="flex-1">
          ...
          <p className="text-white/70 text-sm">Get quick access on your home screen</p>
        </div>
      </div>
      <div className="flex justify-end gap-3 mt-4">
        ...
        <button 
          ...
          className="px-4 py-2 bg-white text-black hover:bg-white/90 transition-colors text-sm font-medium rounded-full"
        >
          Install
        </button>
      </div>
